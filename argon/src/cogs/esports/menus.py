...
class IDPMenu(menus.Menu):
    def __init__(self, send_channel: ArgonTextChannel, role: ArgonRole):
...
class AutocleanMenu(menus.Menu):
    ...
    async def on_three(self, payload):
        msg = await self.ctx.send(
            ...
            "**Currently Argon works according to Indian Standard Time (UTC+05:30)**"
        )
