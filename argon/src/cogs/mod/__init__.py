...
from core import Cog, Context, ArgonView, role_command_check
from models import Lockdown
from utils import ActionReason, BannedMember, FutureTime, MemberID, ArgonUser, emote, human_timedelta, plural
...
class Mod(Cog):
    ...
    @clear.command()
    @commands.has_permissions(manage_messages=True)
    async def user(self, ctx, member: ArgonUser, search=100):
